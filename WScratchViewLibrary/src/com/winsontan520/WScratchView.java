...
public class WScratchView extends SurfaceView implements IWScratchView, SurfaceHolder.Callback {
	 ...
	 public static interface OnScratchScreenListener
	    {

	        public abstract void onScratchScreen(double d);
	    }
	...
	private OnScratchScreenListener onScratchScreenListener;
	private Point p1;
	private Point p2;
...
*/
	...
	public boolean onTouchEvent(MotionEvent me) {
		synchronized (mThread.getSurfaceHolder()) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				startY = me.getY();
				mPathList.add(path);
				 p1 = new Point((int)me.getX(), (int)me.getY());
				break;
			case MotionEvent.ACTION_MOVE:
				double d;
		        p2 = new Point((int)me.getX(), (int)me.getY());
		        d = getDistanceBetweenPoints(p1, p2);
		        p1 = p2;
				if (mScratchStart) {
					path.lineTo(me.getX(), me.getY());
					 onScratchScreenListener.onScratchScreen(1.0D * (4D * d));
				} else {
	...
	public static abstract class OnScratchCallback{
	 ...
	 public void setOnScratchScreenListener(OnScratchScreenListener onscratchscreenlistener)
	    {
	        onScratchScreenListener = onscratchscreenlistener;
	    }
	 
	 private float getDistanceBetweenPoints(float f, float f1, float f2, float f3)
	    {
	        return (float)Math.sqrt(Math.pow(f - f1, 2D) + Math.pow(f2 - f3, 2D));
	    }

	    private float getDistanceBetweenPoints(Point point, Point point1)
	    {
	        return getDistanceBetweenPoints(point.x, point1.x, point.y, point1.y);
	    }
}
